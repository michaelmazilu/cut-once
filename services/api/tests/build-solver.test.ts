...
describe("solve", () => {
  ...
  it("never stands something outside the vocabulary on a smaller face — a bowl on its rim — but will lay it down", () => {
    const bowl = twin({ twin_id: "o9", name: "other", label: "fruit bowl", shape: { type: "box", size: [0.22, 0.08, 0.14] } });
    const crate = twin({ twin_id: "o8", name: "other", label: "cereal box", shape: { type: "box", size: [0.30, 0.20, 0.08] } });
    const things = new Map([...kit, ["o9", bowl], ["o8", crate]]);
    const r = solve(draft([step({ place: "o9", orientation: "upright" })]), things);
    expect(!r.ok && r.reason).toBe("the fruit bowl can be laid down but not stood up, because only its size was measured; keep it flat");
    expect(solve(draft([step({ place: "o9", orientation: "flat" })]), things)).toMatchObject({ ok: true });
    // Found standing on its side, the cereal box may still be laid flat: that is putting it down, not standing it up.
    expect(solve(draft([step({ place: "o8", orientation: "flat" })]), things)).toMatchObject({ ok: true });
    expect(!solve(draft([step({ place: "o8", orientation: "upright" })]), things).ok).toBe(true);
  });
